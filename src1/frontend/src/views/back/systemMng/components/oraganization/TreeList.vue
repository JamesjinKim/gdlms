<template>
  <div class="content-l role">
      ...
      <div class="tree-drag__wrap">
        ...
        >
          <ul>
            ...
            <li>
              <button class="btn-pop" @click.prevent="onIsSelectable">
                사용 중지
              </button>
            </li>
          </ul>
    ...
    <!-- 사용 중지 -->
    <AppDialog
      v-model:view="useIsSelectable.view"
      :message="useIsSelectable.msg"
      @confirm="confirmIsSelectable"
    />
  </div>
</template>
...
***********/
// const removeId = ref('');
const useIsSelectable = reactive({
  view: false,
  msg: '정말로 사용중지 하시겠습니까?',
});
const onIsSelectable = () => {
  selectId.value = activeTooltipId.value;
  const getIsSelectable = getIsSelectableById(treeData.value, selectId.value);
  console.log(getIsSelectable);
  useIsSelectable.msg = getIsSelectable
    ? '정말로 사용중지 하시겠습니까?'
    : '정말로 사용중지를 해지 하시겠습니까?';
  useIsSelectable.view = true;
};
const confirmIsSelectable = () => {
  updateIsSelectable(treeData.value, selectId.value);
  selectId.value = '';
};
//선택 노드 Disabled 상태값
const getIsSelectableById = (tree, selectId) => {
  for (const node of tree) {
    if (node.data.id === selectId) {
      return node.isSelectable;
    }
    if (node.children) {
      const disabledValue = getIsSelectableById(node.children, selectId);
...
//변경 부서 데이터 수정
const updateIsSelectable = (tree, selectId) => {
  for (const node of tree) {
    if (node.data.id === selectId) {
      node.isDraggable = !node.isDraggable;
      node.isSelectable = !node.isSelectable;
      return true;
    }

    if (node.children) {
      const updated = updateIsSelectable(node.children, selectId);
      if (updated) {
        return true;
      }
    }
