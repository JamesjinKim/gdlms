...
# 로그 파일 경로 설정
log_file = os.path.join(log_dir, 'server22_command.log')

# 로거 생성
logger = logging.getLogger("CommandServer22")
...
async def run_server():
    ...
    logger.info(f"Starting Command Server22 on port {port}")
...
if __name__ == "__main__":
    try:
        ...
        logger.info("Server22 stopped by user")
    except Exception as e:
        logger.error(f"Server22 error: {e}")